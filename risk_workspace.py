...
class RiskWorkspace(App):
    ...
    #position-context { background: $surface-darken-2; border: solid $secondary; padding: 1 2; margin-bottom: 1; height: 8; color: $text; }
    ...
    def load_portfolio(self) -> None:
        ...
        for _, row in self.enriched_data.sort_values("Ticker").iterrows():
            ...
            ticker_display = f"[{stop_type_char}] {row['Ticker']}"
            if conid_str in self.drafts:
                ticker_display = f"[bold yellow]* [{self.drafts[conid_str]['type'][:1]}] {row['Ticker']}"
            ...
            r_color = "red" if row['risk_pct_nav'] >= 1.0 else ("yellow" if row['risk_pct_nav'] >= 0.5 else "white")
    ...
    def refresh_risk_checklist(self, hypo_stop: Optional[float] = None) -> None:
        ...
        # 2. Exposure Audit (% NAV <= 5%)
        nav_pct = metrics['NavPct']
        exp_color = "green" if nav_pct <= 5.0 else "red"
        exp_status = "[PASS]" if nav_pct <= 5.0 else "[OVER LIMIT]"
        
        # 3. Integrity Audit (Price > Stop)
        # Use hypo_stop from sandbox if available, else use current SL_Price
        ...
        if pd.notnull(stop_p):
            ...
            integ_content = "[dim]Waiting for Strategy...[/]"

        # 4. Build Output
        audit_text = (
            f"[bold yellow]{pos.ticker}[/] ({pos.name})\n"
            f"--------------------------------------\n"
            f"1. EXPOSURE: [bold {exp_color}]{nav_pct:.1f}%[/] of NAV | {exp_status} [dim](Limit: <= 5%)[/]\n"
            f"2. INTEGRITY: {integ_content}"
        )
    ...
    def on_strategy_change(self) -> None:
        ...
        try:
            match = re.match(r"([0-9\.%]+)\s*([FT]?)", raw_text)
            if not match: return
            val_part, type_char = match.groups()
            ...
            r_color = "red" if hypo_r >= 1.0 else ("yellow" if hypo_r >= 0.5 else "white")
            ...
            table = self.query_one("#portfolio-table")
            table.update_cell(self.current_conid, "col_ticker", f"[bold yellow]* [{stop_type[:1]}] {pos_obj.ticker}")
            ...
            self.drafts[self.current_conid] = {'atr': final_atr, 'type': stop_type, 'ticker': pos_obj.ticker}
        except: pass
    ...
    def on_key(self, event) -> None:
        ...
        if event.key == "ctrl+j":
            if self.current_conid in self.drafts:
                ...
                self.notify(f"COMMITTED: {d['ticker']} @ {d['atr']:.2f}")
                self.query_one("#atr-input").value = ""; self.load_portfolio()
                self.query_one("#portfolio-table").focus()

    def action_save_all(self) -> None:
        if not self.drafts: return
        for conid, draft in self.drafts.items(): set_position_risk(conid, draft['ticker'], draft['atr'], draft['type'])
