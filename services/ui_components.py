            ...
            with TabbedContent(initial="tab-visuals"):
                ...
                with TabPane("Strategy Lab", id="tab-syntax"):
                    with ScrollableContainer(classes="help-scroll"):
                        ...
                        "Extends the top target beyond the default [bold]entry + 3×ATR[/] (3R). The multiple is\n"
                        "of the [bold]Inception ATR[/] (frozen at first save), so the target does NOT drift when\n"
                        "you later tighten the stop. M1/M2 milestones always stay at +1R / +2R.\n\n"
                        "  Command        │ Sets the target to\n"
                        "  ───────────────┼──────────────────────────────────────────────────\n"
                        "  [bold cyan]TP:4[/] / [bold cyan]TP:4R[/]   │ entry + 4×Inception ATR  (the R is optional)\n"
                        "  [bold cyan]TP:+35%[/]        │ entry +35%  →  multiple = (entry×35%)/ATR\n"
                        "  [bold cyan]TP:$60K[/]        │ total open profit = $60,000  (needs share count)\n"
                        "  [bold cyan]TP:3:1[/]         │ auto N:1 forward reward:risk vs the stop you're setting\n"
                        "  [bold cyan]TP:-[/]           │ clears the override → back to default 3R\n\n"
                        "[bold]TP:N:1 — anchor the target to the stop.[/] Sets the target to N units of upside\n"
                        "from the current price for every unit down to the stop: [bold]target = price + N×(price−stop)[/].\n"
                        "It reads the stop from the SAME command, so [bold]@655 T TP:3:1[/] locks a $655 floor and\n"
                        "puts the target exactly 3:1 above price — no manual maths. The result is frozen as an\n"
                        "ATR multiple, so tightening the stop later won't move it; re-run the command to re-anchor.\n\n"
                        "[bold yellow]WHEN to use it — the re-anchor play.[/] A position is opened with an inception ATR\n"
                        "and stop, which fixes the original target at [bold]entry + 3×inception ATR[/] (3R). Once price\n"
                        "REACHES that target the exit ladder is maxed and the panel parks on 'trim'. That is the\n"
                        "trigger to RE-ANCHOR: tighten the stop to lock in the open profit, then set [bold]TP:3:1[/] to\n"
                        "hand the winner a fresh 3:1 runway from here. M1/M2 stay at +1R/+2R; only the top rung moves.\n\n"
                        "[bold]Two equivalent ways to set the protective stop[/] — pick by how you think:\n"
                        "  [bold cyan]@655 T TP:3:1[/]    │ by PRICE  — pin the stop floor at 655.\n"
                        "  [bold cyan]44.13 T TP:3:1[/]   │ by ATR    — keep a 44.13 buffer below the high.\n"
                        "Both store the SAME trailing buffer and trail identically afterward; they only differ at\n"
                        "input time if a new high prints (by-PRICE holds the floor, by-ATR holds the distance).\n\n"
                        "[bold]3:1 guardrail.[/] With an override active the panel shows a [bold]TARGET[/] line with the\n"
                        "FORWARD reward:risk — (target − price)/(price − stop) — flagged [bold red]⚠ below 3:1[/] if it\n"
                        "drops under 3.0. A run-up winner whose stop sits above entry will read low; that\n"
                        "forward ratio is the honest measure once entry risk is gone.\n"
                        "  [dim]e.g. TP:5R → VOO: TARGET 5.0R → 763.98, fwd RR flagged vs 3.0[/]\n\n"
