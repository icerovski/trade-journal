...
- **`core/profit_taking.py`** — `compute_exit_milestones()` sets `m1_price`, `m2_price`, `exit_stage` on a Position (ladder is uniform: M1/M2/TP = entry + 1/2/3×ATR for both stop types). `classify_regime()` is the pure regime decision (TREND ≥21d + price>DMA, NORMAL 10-20d or unconfirmed reversal, RANGING) including reversal hysteresis; `enrich_regime()` fetches the 200-DMA signal and applies it. `TRIM_MATRIX` maps `(stage, regime) → (fraction, rationale)`; a `0.0` fraction means hold (e.g. M2 in TREND). Called from `portfolio_manager.get_dashboard_df()` step 4.
...
**Exit Milestones & Regime:** Milestones computed in `profit_taking.compute_exit_milestones()`; regime in `profit_taking.enrich_regime()`/`classify_regime()` (step 4 of `get_dashboard_df`). TP is entry-anchored for both stop types (uniform ladder). Full thresholds and trim guidance in `docs/TECHNICAL_DOCS.md` §5.
