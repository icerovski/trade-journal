...
- **`core/zone_scan.py`** — Entry/Exit Zone Scanner orchestrator. `scan_ticker()` / `build_zone_report()` merge VP + AVWAP + DMAs into one confluence read, pick a stop (nearest support, or momentum micro-structure), and size under all presets; I/O is injected via a `price_loader`. **Momentum Regime:** when price runs >`MOMENTUM_VAL_PREMIUM_PCT` above the 6mo VAL, the stop switches to a micro-structure level (`MICRO_LOOKBACK_DAYS` window VAL / swing-low AVWAP, minus `MICRO_STOP_BUFFER_ATR`) and the row is tagged `ZONE-MOMO`.
