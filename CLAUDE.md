...
- **`ui/risk_workspace.py`** — ACTION column uses asymmetric thresholds: ≥10% budget remaining triggers Add, ≥5% triggers Trim (filters transaction noise). Command syntax: `VALUE [F/T] [P:S/B/L] [R:x] [E:x] [TP:n] [C:TH/TE] [G:gap] [X:H/R/T]` — `TP:n` take-profit override; `C:` THESIS/TECHNICAL tag (§0a, carried); `G:` plausible gap price → opt-in gap-aware sizing (§6); `X:` exit shape (§5a). Drafting workflow holds bulk updates in-memory before committing. On commit, `_gate_check` runs entry gates per the `gates_mode` setting (`off|advisory|blocking`, default `off`, toggled in the `M` preset/settings modal); a classified commit writes a `trade_log` entry. PLAN section includes full exit stage and regime breakdown.
...
- **`settings`** — key/value app settings. Keys: `action_threshold_pct`, `gates_mode` (`off|advisory|blocking`), `calibration_profile` (`default|position_3to6mo`).
