...
The resolver turns a user token into a multiple of the inception ATR. All accepted forms
(R-multiple, % gain, clear) plus the unresolvable cases are covered here. The absolute-$
form was cut (assessment review, 2026-07-04) — $ / K tokens must now be rejected. Tokens
...
def test_resolve_plain_multiple():
    mult, clear, ok = resolve_tp_mult("4", ENTRY, INC_ATR)
    assert ok and not clear and mult == pytest.approx(4.0)


def test_resolve_r_suffix():
    mult, clear, ok = resolve_tp_mult("4R", ENTRY, INC_ATR)
    assert ok and not clear and mult == pytest.approx(4.0)
...
def test_resolve_percent():
    ...
    for token in ("+35%", "35%"):
        mult, clear, ok = resolve_tp_mult(token, ENTRY, INC_ATR)
        assert ok and not clear and mult == pytest.approx(7.0)


def test_dollar_forms_rejected():
    """The absolute-$ form was cut: $ / K tokens resolve to ok=False (warn-and-ignore)."""
    for token in ("$600", "$6K", "60K"):
        mult, clear, ok = resolve_tp_mult(token, ENTRY, INC_ATR)
        assert not ok and mult is None


def test_resolve_clear():
    mult, clear, ok = resolve_tp_mult("-", ENTRY, INC_ATR)
...
def test_resolve_needs_inception_atr():
    ...
    mult, clear, ok = resolve_tp_mult("4", ENTRY, 0.0)
    assert not ok


def test_resolve_garbage_token():
    mult, clear, ok = resolve_tp_mult("XYZ", ENTRY, INC_ATR)
    assert not ok
