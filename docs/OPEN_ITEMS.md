...
- [ ] Delete the ghost hub `OneDrive\Accounts\HTC_EOOD\TradeJournalData` — confirmed to hold nothing unique (its `prices.db` has 7/31–8/3, which re-syncs).
- [ ] OneDrive conflict copies in the data hub: `trade_journal-LAPTOP-20V5N4Q9.db` plus four conflicted `.log` files, from two machines (May–Jul). A conflict copy of a SQLite ledger is a silent fork, not a duplicate file. Decide a single-writer rule or move the DB off the synced folder.
