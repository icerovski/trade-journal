...
- [x] Confirm the configured `DATA_PATH` is the intended hub — it was **not**. `.env` pointed at `OneDrive\Accounts\HTC_EOOD\TradeJournalData`, an empty hub auto-created by `config.mkdir` and filled by `init_db` + a YTD-only CSV ingest (488 trades, 0 profiles). The live book is `OneDrive\Companies\...` (2,306 trades from 2024-09, 54 ACTIVE profiles, `gates_mode=advisory`, `regime_lens=horizon`). Repointed 2026-08-04, both `.env` copies. **Left over:** `Accounts\` still holds the stray hub — delete it once you are satisfied nothing unique landed there (its `prices.db` has 4 days the live one lacks, which will simply re-sync).
